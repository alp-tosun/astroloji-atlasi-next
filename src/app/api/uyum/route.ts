...
import { uyumSchema } from '@/lib/validation/schemas';
...
export async function POST(req: NextRequest) {
  const guard = await withApiGuards(req, uyumSchema, { rateLimit: 'ai', premium: true });
  if ('response' in guard) return guard.response;
  try {
    const { stream, onComplete } = await useCases.uyum.execute(guard.data, guard.uid);
    return streamResponse(stream, onComplete);
  } catch (e) {
    console.error('/api/uyum:', e);
